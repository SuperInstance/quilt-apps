...
export function createPatchWall(id: string = 'patchwall'): PatchWallCell {
  return { id, points: new Map(), links: [], tickCount: 0 };
}

export function addPoint(cell: PatchWallCell, tag: string, type: PointCell['type'], location: string, description: string = ''): PointCell {
  const p: PointCell = {
...
export function linkPoints(cell: PatchWallCell, fromTag: string, toTag: string, linkType: LineageLink['type']): boolean {
...
export function upstream(cell: PatchWallCell, tag: string): PointCell[] {
  ...
  function walk(t: string) {
    ...
    for (const u of upstreamOf) walk(u.tag);
    const pt = cell.points.get(t);
    if (pt && t !== tag) out.push(pt);
  }
  for (const t of upstreamTags) walk(t);
  return out;
}

export function downstream(cell: PatchWallCell, tag: string): PointCell[] {
  const p = cell.points.get(tag);
  if (!p) return [];
  const outgoing = cell.links.filter(l => l.from === p.id);
  const downstreamIds = new Set<string>();
  for (const l of outgoing) downstreamIds.add(l.to);
  const visited = new Set<string>();
  const out: PointCell[] = [];
  function walk(id: string) {
    ...
    for (const sl of subLinks) walk(sl.to);
  }
  for (const id of downstreamIds) walk(id);
  return out;
}

export function impact(cell: PatchWallCell, tag: string): { direct: PointCell[]; cascade: PointCell[] } {
...
export function importCSV(cell: PatchWallCell, csv: string): number {
  ...
  for (const line of lines) {
    ...
    if (!['sensor', 'tag', 'alarm', 'consumer', 'controller'].includes(type)) continue;
    addPoint(cell, tag, type as PointCell['type'], location || '', description || '');
    added++;
  }
  // Second pass: add links
  for (const line of lines) {
    const [tag, , , , linksStr] = line.split(',').map(s => s.trim());
    if (!tag || !linksStr) continue;
    for (const linkSpec of linksStr.split('|')) {
      const [target, ltype] = linkSpec.split(':');
      if (target && ltype) {
        linkPoints(cell, tag, target, ltype as LineageLink['type']);
      }
    }
  }
  return added;
...
export function toJSON(cell: PatchWallCell): { points: any[]; links: any[] } {
  return {
...
export function renderGraph(cell: PatchWallCell): string {
...
export function pointCount(cell: PatchWallCell): number {
  return cell.points.size;
}

export function linkCount(cell: PatchWallCell): number {
  return cell.links.length;
}

export function blastRadius(cell: PatchWallCell, tag: string): PointCell[] {
  return downstream(cell, tag);
}

export function rootPoints(cell: PatchWallCell): PointCell[] {
...
export function leafPoints(cell: PatchWallCell): PointCell[] {
